...
if showboxplot:
    df = pandas.DataFrame(featureFormat(data_dict, ['poi_to_ratio','poi'], sort_keys = False,remove_NaN=False),columns=['poi_to_ratio','poi'])
    df.boxplot(by='poi')
    plt.show()

    df = pandas.DataFrame(featureFormat(data_dict, ['poi_from_ratio','poi'], sort_keys = False,remove_NaN=False),columns=['poi_from_ratio','poi'])
    df.boxplot(by='poi')
    plt.show()
...
if performalltunings:
    ...
    # Let's have a look at the explained variance of the PCA algorithm
    # For submission v2.
    explained_variance = optim_clf.named_steps['pca'].explained_variance_
    plt.figure(GetSequence())
    plt.plot(explained_variance,'bs')
    plt.xlabel('Component')
    plt.ylabel('Explained variance')
    plt.show()

    # Dump classifier and associated data for algorithm check
